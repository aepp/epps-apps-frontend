...
export const downloadCV = (): ThunkAction<
  void,
  RootState,
  null,
  Action<string>
> => (dispatch: ThunkDispatch<{}, {}, any>, getState) => {
  ...
  html2canvas(cvContainer, {
    ...
    .then(canvas => {
      ...
      pdf
        .save('cv.pdf', {returnPromise: true})
        .then(() => dispatch({type: FINISH_CV_DOWNLOAD}));
    });
};

export const changeDesign = (designSchemeId: DesignSchemeIdType) => ({
  type: CHANGE_DESIGN,
